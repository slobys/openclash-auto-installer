...
- `nikki.sh`：安装或更新 Nikki；采用轻刷新模式，不强制断开当前 LuCI 会话；在 OpenWrt/opkg 环境下会先检测 `/etc/nikki/ucode/include.uc` 等关键文件，若主包状态存在但文件缺失则自动强制重装 `nikki` 主包，再调用 Nikki 官方 `install.sh` 完成初始化，并补装中文语言包；若初次显示为英文，刷新页面后中文语言包会自动生效
- `menu.sh`：菜单式管理入口
