...
- `nikki.sh`：安装或更新 Nikki；采用轻刷新模式，不强制断开当前 LuCI 会话；在 OpenWrt/opkg 环境下改为调用 Nikki 官方 `install.sh` 完成初始化，再补装中文语言包，避免仅装包导致 `/etc/nikki` 运行目录缺失；若初次显示为英文，刷新页面后中文语言包会自动生效
- `menu.sh`：菜单式管理入口
